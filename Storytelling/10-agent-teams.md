...
Subagent(Ch06)를 쓰면? 부모가 기다려야 한다. 수직적 구조. 코드 리뷰를 맡길 수는 있지만, **프론트엔드와 백엔드를 동시에 구현하게 할 수는 없다.**

Background Tasks(Ch07)를 쓰면? I/O만 병렬화된다. `npm install`은 뒤에서 돌리면서 코드를 쓸 수 있지만, **LLM 추론 자체는 여전히 하나.**
...
## Task Board: 05에서 배운 그것

Agent Teams의 "작업 보드"는 Ch05에서 배운 **Task System 그 자체**다.
...
> Ch07의 BackgroundManager에서 배운 "진동벨" 패턴과 같은 원리다. 쌓아두고, 확인할 때 한 번에 수거.
...
```
Ch06 (Subagent): 수직적 격리
  부모 → 자식 (깨끗한 컨텍스트)
  자식 → 부모 (요약 반환)
  부모가 기다린다.

Ch12 (Agent Teams): 수평적 격리
  에이전트A ──┐
