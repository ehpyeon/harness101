...
지민은 Agent Teams(Ch12)를 써서 세 에이전트를 동시에 돌렸다.
...
Worktree를 Task System(Ch05)과 연결하면 강력한 구조가 나온다:
...
Ch03(Hooks)의 Permissions 레이어가 여기서도 작동한다.
...
Ch06(Agents)에서 **메시지를 격리**했다. `messages=[]`로 시작해서 노이즈를 차단.
